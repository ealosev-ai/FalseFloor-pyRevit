...
3. Read `pythonnet.md` when a task involves `clr`, `System.*`, WPF, or .NET interop behavior.
4. Read `clipper2.md` when a task touches exact contour geometry, clipping, or offset logic.
5. Read `github-actions.md` when a task touches CI, packaging, or workflow behavior.
6. Read `pyyaml.md` when a task plans YAML-backed settings or presets.
7. Open `dynamo.md` only if the task really involves Dynamo graphs or Dynamo interop.
8. For exact signatures, always prefer the docs for the Revit version that is actually running.
